...
-tweaked PLC and PLR decisions so all cores are carried over\
(In GFM, PLC and PLR get all cores from their former, but then lose some cores. I disagree with this.)\

\
*/events/*\
-new event chain for Poland if it is winning the November Uprising by 4% or greater\
--Republic path: Poland gets lots of prestige but Prussia and Austria will declare war\
--Regency path: plurality boost, later get event for Prince Radziwiłł to become king\
